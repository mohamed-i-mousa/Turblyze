...
void KOmegaSST::calculateTurbulentViscosity(const VectorField&,
                                           const std::vector<VectorField>& gradU,
                                           Scalar rho,
                                           Scalar mu_lam) {
...
void KOmegaSST::calculateBlendingFunctions(const std::vector<VectorField>&,
                                          Scalar rho,
                                          Scalar mu_lam) {
    /**
    ...
    for (size_t i = 0; i < allCells.size(); ++i) {
        ...
        Scalar CDkw = std::max(2.0 * rho * constants.sigma_omega2 / omega[i] * dot_product, 1e-10);
        Scalar arg1_3 = 4.0 * rho * constants.sigma_omega2 * k[i] / (CDkw * y * y);
